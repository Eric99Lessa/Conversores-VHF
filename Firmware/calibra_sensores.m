...
t_1 = t(idx_parte1);
t_2 = t(idx_parte2);
Vin_1 = Vin(idx_parte1);
Vin_2 = Vin(idx_parte2);
Vout_1 = Vout(idx_parte1);
Vout_2 = Vout(idx_parte2);
V_mult_1 = V_mult(1:6);
...
[t_1_fill_Vout, Vout_1_filt, means_Vout_1] = filt_step_conv(t_1, Vin_1, step_dot);
...
means_Vin = [means_Vin_1; means_Vin_2];
means_Vout = [means_Vout_1; means_Vout_2];

p_Vin = polyfit(means_Vin, V_mult(1:end-1), 1);
p_Vout = polyfit(means_Vout, V_mult(1:end-1), 1);

figure(6)
subplot(2, 1, 1)
plot(means_Vin, V_mult(1:end-1), 'lineWidth', 2); hold on;
plot(means_Vin, polyval(p_Vin, means_Vin), 'lineWidth', 2);
subplot(2, 1, 2)
plot(means_Vout, V_mult(1:end-1), 'lineWidth', 2); hold on;
plot(means_Vout, polyval(p_Vout, means_Vout), 'lineWidth', 2);

figure(7)

subplot(3, 1, 1)
plot(t, polyval(p_Vin, Vin), 'LineWidth', 2); hold on;
grid on; grid minor;
xlabel('Tempo (s)');
ylabel('Tensão (V)');
title('Tensão de entrada')
...
subplot(3, 1, 2)
plot(t, polyval(p_Vout, Vout), 'LineWidth', 2); hold on;
grid on; grid minor;
xlabel('Tempo (s)');
ylabel('Tensão (V)');
title('Tensão de saída')
