...
{
    duty_u = clampf(duty_u, 0.0f, 70.0f);
    duty_v = clampf(duty_v, 0.0f, 70.0f);
    duty_w = clampf(duty_w, 0.0f, 70.0f);
                                         ...
                                         uint16_t duty_out[3])
{
    ...
    // Get states
    float Vin = state->x[0];
    float Vout = state->x[1];
    float IL_1 = state->x[2];
    float IL_2 = state->x[3];
    float IL_3 = state->x[4];
    ...
    float d_1 = 1 - (Vin - IL_1 * R_L + Kp_i * L * (IL_1 - IL_ref)) / (VD + Vout);
    float d_2 = 1 - (Vin - IL_2 * R_L + Kp_i * L * (IL_2 - IL_ref)) / (VD + Vout);
    float d_3 = 1 - (Vin - IL_3 * R_L + Kp_i * L * (IL_3 - IL_ref)) / (VD + Vout);
    duty_out[0] = 100 * d_1;
    duty_out[1] = 100 * d_2;
    duty_out[2] = 100 * d_3;
}
...
void mcpwm_control_task(void *arg)
{
    ...
    while (1)
    {
        ...
        taskENTER_CRITICAL(&telem_mux);
        float duty_u = clampf(duty_perm[0], 0.0f, 70.0f);
        float duty_v = clampf(duty_perm[1], 0.0f, 70.0f);
        float duty_w = clampf(duty_perm[2], 0.0f, 70.0f);
        g_telem_state.duty[0] = duty_u;
        g_telem_state.duty[1] = duty_v;
        g_telem_state.duty[2] = duty_w;
        taskEXIT_CRITICAL(&telem_mux);
